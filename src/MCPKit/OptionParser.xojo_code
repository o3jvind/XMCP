...
Protected Class OptionParser
	#tag Method, Flags = &h0, Description = 4164647320616E206F7074696F6E20746F20746865207061727365722E
		Sub AddOption(shortKey As String, longKey As String, description As String, type As OptionTypes, isRequired As Boolean = False)
		  /// Adds an option to the parser.
		  ///
		  /// An `OptionParserException` can be thrown if one attempts to add an option with the same
		  /// short or long key as an existing option.
		  
		  #Pragma BreakOnExceptions False
		  
		  // Validation.
		  If shortKey <> "" And Dict.HasKey(shortKey.Asc) Then
		    Raise New OptionException("You can't add the same short key more than once: " + shortKey)
		  End If
		  If longKey <> "" And Dict.HasKey(longKey) Then
		    Raise New OptionException("You can't add the same long key more than once: " + longKey)
		  End If
		  If shortKey = "?" Then
		    Raise New OptionException("You can't add the key ""?"" This means ""help"" and has already " + _
		    "been added for you")
		  End If
	...
	#tag Method, Flags = &h0
		Sub Constructor(appDescription As String = "")
		  //
	...
	#tag Method, Flags = &h0
		Sub ShowHelp(sectionTitle As String = "Help")
		  ...
		  // * For a GUI application, the output is given to `MessageBox`
		  ...
		  Var helpFor As String = App.Name
		  If helpFor <> "" Then
